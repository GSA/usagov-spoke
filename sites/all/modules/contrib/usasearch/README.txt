...
6. After a configuration change, run Re-Index Site on admin/config/search/settings, and then run cron.
   If using Rules a rule will be needed to re-index the site on cron.
...
#### Rules Integration
All indexing can be handled with Rules by checking "Use rules to index content" on the
DigitalGov Search configuration page. If selected all indexing is handled by Rules
including re-indexing the site.
